...
func generateSignature(incPhoneNumber string, incClientID string, incVariousID string) string {
	...
	_, encPhoneNumber := SimpleStringEncrypt(incPhoneNumber, incVariousID)
	_, encClientID := SimpleStringEncrypt(incClientID, incVariousID)

	rawSignature := encPhoneNumber + "||" + encClientID + "||" + incVariousID
	_, strSignature := SimpleStringEncrypt(rawSignature, strSalt)
	...
	if err != nil {
		...
		for rows.Next() {
			...
			if errS != nil {
				...
				DoLog("INFO", incTraceID, "Modules", "RedisSession",
...
func ValidateRedisSession(db *sql.DB, redisClient *redis.Client, contextX context.Context, incTraceID string, incClientID string, incPhoneNumber string, incSessionID string) (string, string) {
	...
	if err == nil {
		DoLog("INFO", incTraceID, "Modules", "Validate",
		...
		if len(strSignature) > 0 {
			...
			if strSignature == incSessionID {
				...
				strSignatureX := SimpleStringDecrypt(strSignature, strSalt)
				arrSignature := strings.Split(strSignatureX, "||")
				decPhoneNumber := SimpleStringDecrypt(arrSignature[0], arrSignature[2])
				decClientiD := SimpleStringDecrypt(arrSignature[1], arrSignature[2])
			...
			DoLog("ERROR", incTraceID, "Modules", "Validate",
